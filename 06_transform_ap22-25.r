...
base::print("% Changing datatypes in apistrip spain data.")

#tbl_samples_a_sp
tbl_samples_a_sp$sample_date <- base::as.Date(tbl_samples_a_sp$sample_date, format = "%d.%m.%Y")
tbl_samples_a_sp$colony <- base::as.character(tbl_samples_a_sp$colony)
...
tbl_results_bb <- myfun.add_experiment_to_results(fun_input_table = tbl_results_bb,
                                                  ...
                                                  fun_matrix = "Bee bread")

base::print("% Joining beebread results with matrix information.")
tbl_results_bb <- tbl_results_bb %>% 
  dplyr::inner_join(tbl_matrixes, by = dplyr::join_by(matrix == matrix))
    ...
    for (j in base::seq_along(myvar.unique_locations_bb_results)) {
      ...
      for (k in base::seq_along(myvar.unique_substances_bb_results)) {
        ...
        tbl_avg_cum_bb[nrow( tbl_avg_cum_bb) + 1,] = list(tbl_tmp_results_substances$fk_id_sam_bb[1],
                                                          ...
                                                          tbl_tmp_results_substances$matrix_short[1],
...
base::print("% Joining pollen results with matrix information.")
tbl_results_p <- tbl_results_p %>% 
  dplyr::inner_join(tbl_matrixes, by = dplyr::join_by(matrix == matrix))
...
base::print("% Joining wax results with matrix information.")
tbl_results_w <- tbl_results_w %>% 
  dplyr::inner_join(tbl_matrixes, by = dplyr::join_by(matrix == matrix))
...
base::print("% Joining apistrip_l2 results with sample list.")

tbl_results_a <- tbl_results_a %>% 
  dplyr::inner_join(tbl_samples_a, by = dplyr::join_by(fk_id_sam_a == pk_id_sam_a))

base::print("% Joining apistrip_l2 results with location information.")

tbl_results_a <- tbl_results_a %>% 
  dplyr::inner_join(tbl_locations, by = dplyr::join_by(location == location_long))

base::print("% Joining apistrip_l2 results with experiment information.")

#add information from tbl_experiments to tbl_results
tbl_results_a <- myfun.add_experiment_to_results(fun_input_table = tbl_results_a,
                                                 fun_date_column = tbl_results_a$sample_date,
                                                 fun_matrix = "Apistrip_L2")

base::print("% Joining apistrip_l2 results with matrix information.")
tbl_results_a <- tbl_results_a %>% 
  dplyr::inner_join(tbl_matrixes, by = dplyr::join_by(matrix == matrix))
...
base::print("% Transforming apistrip_l2 results table to long format.")
...
base::print("% Adding LOD and LOQ information to apistrip_l2 results.")
...
base::print("% Checking LOD for apistrip_l2 results.")

#check if concentration is greater than LOD
tbl_results_a <- myfun.check_lod(tbl_results_a)

base::print("% Checking LOQ for apistrip_l2 results.")
...
base::print("% Creating new tibble for apistrip_l2 mean and cumulative values.")
...
base::print("% Joining apistrip_l1 results with sample list.")

tbl_results_a_sp <- tbl_results_a_sp %>% 
  dplyr::inner_join(tbl_samples_a_sp, by = dplyr::join_by(fk_id_sam_a_sp == pk_id_sam_a_sp))

base::print("% Joining apistrip_l1 results with location information.")

tbl_results_a_sp <- tbl_results_a_sp %>% 
  dplyr::inner_join(tbl_locations, by = dplyr::join_by(location == location_long))

base::print("% Joining apistrip_l1 results with experiment information.")

#add information from tbl_experiments to tbl_results
tbl_results_a_sp <- myfun.add_experiment_to_results(fun_input_table = tbl_results_a_sp,
                                                 fun_date_column = tbl_results_a_sp$sample_date,
                                                 fun_matrix = "Apistrip_L1")


base::print("% Joining apistrip_l1 results with matrix information.")
tbl_results_a_sp <- tbl_results_a_sp %>% 
  dplyr::inner_join(tbl_matrixes, by = dplyr::join_by(matrix == matrix))



base::print("% Transforming apistrip_l1 results table to long format.")

#pivot longer
tbl_results_a_sp <- tbl_results_a_sp %>% 
  tidyr::pivot_longer(
    cols = "Acetamiprid":"Trifloxystrobin", 
    names_to = "substance", 
    values_to = "concentration",
    values_drop_na = FALSE
  )
...
#replace all NA values with 0
tbl_results_a_sp$concentration <- base::replace(tbl_results_a_sp$concentration, base::is.na(tbl_results_a_sp$concentration), 0)



#create new columns for lod and loq information
tbl_results_a_sp <- tibble::rowid_to_column(tbl_results_a_sp, "ID")
tbl_results_a_sp <- dplyr::mutate(tbl_results_a_sp, 
                               lod = NA,
                               loq = NA,
                               greater_than_lod = NA,
                               greater_than_loq = NA)

base::print("% Adding LOD and LOQ information to apistrip_l1 results.")

#add lod and loq to tbl_results_a by checking tbl_limits
myvar.unique_meth <- base::unique(tbl_results_a_sp$fk_method)
for (i in base::seq_along(myvar.unique_meth)) {
  tmp_tbl_res_meth <- dplyr::filter(tbl_results_a_sp, fk_method == myvar.unique_meth[i])
  for (j in 1:base::NROW(tmp_tbl_res_meth)) {
    myvar.cur_row <- tmp_tbl_res_meth$ID[j]
    myvar.cur_sub <- tmp_tbl_res_meth$substance[j]
    tmp_tbl_lim <- dplyr::filter(tbl_limits, fk_method_id == myvar.unique_meth[i], substance_name == myvar.cur_sub)
    tbl_results_a_sp$lod[myvar.cur_row] <- tmp_tbl_lim$lod[1]
    tbl_results_a_sp$loq[myvar.cur_row] <- tmp_tbl_lim$loq[1]
  }
}
rm(i, j)
rm(myvar.unique_meth,
...
base::print("% Checking LOD for apistrip_l1 results.")

#check if concentration is greater than LOD
tbl_results_a_sp <- myfun.check_lod(tbl_results_a_sp)

base::print("% Checking LOQ for apistrip_l1 results.")

#check if concentration is greater than LOQ
tbl_results_a_sp <- myfun.check_loq(tbl_results_a_sp)
...
base::print("% Creating new tibble for apistrip_l1 mean and cumulative values.")

tbl_avg_cum_a_sp <- dplyr::tibble(id = NA,
                               matrix = NA,
                               matrix_short = NA,
                               location = NA,
                               year = NA,
...
tbl_tmp_results_a_sp <- tbl_results_a_sp
tbl_tmp_results_a_sp[tbl_tmp_results_a_sp == 0.00001] <- 0
  ...
  for (i in base::seq_along(myvar.unique_dates_a_results)) {
    ...
    for (j in base::seq_along(myvar.unique_locations_a_results)) {
      ...
      for (k in base::seq_along(myvar.unique_substances_a_results)) {
        ...
        tbl_avg_cum_a_sp[nrow(tbl_avg_cum_a_sp) + 1,] = list(tbl_tmp_results_substances$fk_id_sam_a_sp[1],
                                                        tbl_tmp_results_substances$matrix[1],
                                                        tbl_tmp_results_substances$matrix_short[1],
                                                        tbl_tmp_results_substances$location[1],
                                                        tbl_tmp_results_substances$year[1],
...
tbl_avg_cum_a_sp <- tbl_avg_cum_a_sp[-1,]
...
base::print("% Creating new tibble for apistrip_spain prevalence information.")

# tbl_prevalence_a --------------------------------------------------------

tbl_prevalence_a_sp <- myfun.create_tbl_prevalence(input_table = tbl_results_a_sp,
                                                output_table = tbl_prevalence_a_sp)


# tbl_percentage_a --------------------------------------------------------

tbl_percentage_a_sp <- myfun.create_tbl_percentage(input_table = tbl_prevalence_a_sp,
                                                output_table = tbl_percentage_a_sp)
...
tbl_avg_cum_agr <- dplyr::bind_rows(tbl_avg_cum_bb, tbl_avg_cum_p, tbl_avg_cum_w, tbl_avg_cum_a, tbl_avg_cum_a_sp)
...
tbl_matrix_diff <- dplyr::tibble(substance = NA,
                                 ...
                                 as_l2_loq = NA,
                                 as_l2_n = NA,
                                 as_l2_prct_gt_loq = NA,
                                 as_l2_max_concentration = NA,
                                 as_l2_mean_concentration = NA,
                                 as_l1_sp_loq = NA,
                                 as_l1_n = NA,
                                 as_l1_prct_gt_loq = NA,
                                 as_l1_max_concentration = NA,
                                 as_l1_mean_concentration = NA)
...
for (i in base::seq_along(myvar.unique_year)) {
  ...
  for (j in base::seq_along(myvar.unique_substance)) {
    ...
    for (h in seq_along(myvar.unique_location)) {
      ...
      tmp_tbl_avg_cum_a_sp <- dplyr::filter(tbl_avg_cum_a_sp, location_short == myvar.unique_location[h], year == myvar.unique_year[i], substance == myvar.unique_substance[j])
      ...
      tmp_tbl_prevalence_a_sp <- dplyr::filter(tbl_prevalence_a_sp, location_short == myvar.unique_location[h], year == myvar.unique_year[i], substance == myvar.unique_substance[j])
      ...
      tmp_tbl_results_a_sp <- dplyr::filter(tbl_results_a_sp, location_short == myvar.unique_location[h], year == myvar.unique_year[i], substance == myvar.unique_substance[j], greater_than_loq == TRUE)
      ...
      tmp_tbl_results_lod_a_sp <- dplyr::filter(tbl_results_a_sp, location_short == myvar.unique_location[h], year == myvar.unique_year[i], substance == myvar.unique_substance[j])
      ...
      tbl_matrix_diff[nrow(tbl_matrix_diff) + 1,] = list(myvar.unique_substance[j],
                                                         ...
                                                         tmp_tbl_prevalence_bb$prct_gt_loq[1],
                                                         ...
                                                         tmp_tbl_prevalence_p$prct_gt_loq[1],
                                                         ...
                                                         tmp_tbl_prevalence_w$prct_gt_loq[1],
                                                         ...
                                                         tmp_tbl_prevalence_a$prct_gt_loq[1],
                                                         base::max(tmp_tbl_results_a$concentration),
                                                         base::mean(tmp_tbl_results_a$concentration),
                                                         tmp_tbl_results_lod_a_sp$loq[1],
                                                         tmp_tbl_prevalence_a_sp$prevalence_gt_loq[1],
                                                         tmp_tbl_prevalence_a_sp$prct_gt_loq[1],
                                                         base::max(tmp_tbl_results_a_sp$concentration),
                                                         base::mean(tmp_tbl_results_a_sp$concentration))
